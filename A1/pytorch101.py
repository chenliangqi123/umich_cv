...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # list1 = [x * 10 for x in range(int((start-0.1) // 10 + 1), int((stop - 0.1) // 10 + 1))]
    # x = torch.tensor(list1).double()

    # 使用 torch.arrange 生成指定范围的所有整数
    range_tensor = torch.arange(start, stop, dtype=torch.int64)
    # 过滤出10的倍数
    multiples_of_ten_tensor = range_tensor[range_tensor % 10 == 0]
    # 将张量转换为 torch.float64 类型
    multiples_of_ten_tensor = multiples_of_ten_tensor.to(torch.float64)
    x = multiples_of_ten_tensor
...
def shuffle_cols(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idx0 = torch.tensor([0, 0, 2, 1], dtype=torch.int64)
    y = x[:, idx0]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    # y.scatter_(1, torch.tensor(x).unsqueeze(1), 1.0)
    y.scatter_(1, torch.tensor(x).view(len(x), 1), 1.0)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    min_value, min_index = y.min(dim=1)
    idx0 = torch.arange(y.size(0))
    y[idx0, min_index] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mean_col = x.sum(dim=0) / x.shape[0]
    sample_variance = ((x - mean_col) ** 2).sum(dim=0) / (x.shape[0] - 1)
    sigma = sample_variance.sqrt()
    y = (x - mean_col) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
